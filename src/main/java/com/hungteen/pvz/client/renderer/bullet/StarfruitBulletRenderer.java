...
public class StarfruitBulletRenderer extends EntityRenderer<StarfruitBullet> {
    private final StarfruitBulletModel model;
    private static final ResourceLocation MODEL = Util.prefix("textures/entity/bullet/starfruit.png");
    private static final ResourceLocation ITEM = Util.prefix("textures/item/starfruit.png");

    public StarfruitBulletRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new StarfruitBulletModel<>(context.bakeLayer(PVZLayerHandler.LayerLocationMap.get("starfruit_bullet:main")));
    }

    public void render(StarfruitBullet bullet, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(StarfruitBullet bullet) {
        if (PVZConfig.renderBulletAsModel()) {
            return MODEL;
        } else {
            return ITEM;
