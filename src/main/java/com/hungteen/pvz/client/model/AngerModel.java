...
@OnlyIn(Dist.CLIENT)
public class AngerModel extends HierarchicalModel<Anger> implements ArmedModel {
    ...
    public AngerModel(ModelPart p_233312_) {
    ...
    public void setupAnim(Anger anger, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        float f = ageInTicks * 20.0F * ((float)Math.PI / 180F) + limbSwingAmount;
        float f1 = Mth.cos(f) * (float)Math.PI * 0.15F;
        float f2 = ageInTicks - (float)anger.tickCount;
