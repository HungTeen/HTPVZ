...
public class FireImp extends Imp {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide && this.getPose() == Pose.CROAKING) {
            level.addParticle(ParticleTypes.LAVA, getX(), getY() + 1.3F, getZ(),
    ...
    protected void addBehaviourGoals() {
        ...
        this.targetSelector.addGoal(1, new FireImpShootGoal(this));
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putInt("skill_cooldown", this.tickCount % 200);
    }
    @Override
    ...
    static class FireImpShootGoal extends Goal {
        final Mob zombie;
        ...
        public boolean canUse() {
            return zombie.tickCount % 300 < 220;
        }

        @Override
        public void tick() {
            if (zombie.tickCount % 300 > 200 && zombie.getPose() != Pose.STANDING) {
                zombie.setPose(Pose.STANDING);
            }
            if (EntityUtil.isEntityValid(zombie.getTarget()) && zombie.level.getEntities(this.zombie, zombie.getBoundingBox().inflate(4, 2, 4), entity -> EntityUtil.checkCanEntityBeAttack(zombie, entity)).isEmpty()) {
                if (zombie.tickCount % 300 <= 200 && zombie.getPose() != Pose.CROAKING) {
                    zombie.setPose(Pose.CROAKING);
                }
            }
            if (! zombie.isFreezing() && zombie.getPose() == Pose.CROAKING) {
