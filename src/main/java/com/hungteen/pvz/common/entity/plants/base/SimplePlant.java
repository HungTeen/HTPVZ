...
public class SimplePlant extends Mob implements IHaveSkills, IPlant, ICanAttack {
    ...
    protected static final EntityDataAccessor<Integer> ATTACK_TIME = SynchedEntityData.defineId(SimplePlant.class, EntityDataSerializers.INT);
    ...
    public static boolean onBeingShoveled(Player player, InteractionHand handIn, LivingEntity target) {
        ...
        if (owner != null) {
            permission = PVZConfig.PVZGameRules.getBoolean(player.level, PVZConfig.Common.shovelPermission) ?
            ...
            permission = PVZConfig.PVZGameRules.getBoolean(player.level, PVZConfig.Common.shovelPermission) && EntityUtil.isTeammate(target, player);
        }
    ...
    public static boolean checkSpawnRules(EntityType<? extends LivingEntity> entityType, ServerLevelAccessor level, MobSpawnType mobSpawnType, BlockPos pos, RandomSource random) {
        return entityType instanceof IWaterPlant ? level.getBlockState(pos).getFluidState().is(FluidTags.WATER) : level.getBlockState(pos.below()).is(PVZBlockTags.PLANTABLE_DIRT);
    }
